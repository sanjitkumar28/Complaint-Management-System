...
class SignupAdmin extends React.Component {
    ...
    async signup() {
        try {
            var res = await axios.post('http://localhost:8000/admins', {
                admin_email: this.state.email,
                admin_password: this.state.password,
                admin_name: this.state.name,
                admin_phone: this.state.phone,
                admin_address: this.state.address
            })
            if (res.status == 200) {
                this.setState({
                    loading: false,
                        ...
                        <div className="col-lg-5 col-md-7">
                            ...
                            <div className="row mt-3">
                                ...
                                <div className="col-4 text-center">
                                    <Link to="/signup">
                                        <small style={{ color: 'white' }}>Register as User</small>
                                    </Link>
                                </div>
