...
import LinearProgress from '@material-ui/core/LinearProgress';
 ...
 class Login extends React.Component {
    ...
    //          this.props.history.push('/compform')
        
    //      if (localStorage.('admin_id').length()>0)
    //          this.props.history.push('/selectcomp')
        
    //  }

     async login() {
         try{
             var res = await axios.post('http://localhost:8000/userslogin',{user_email:this.state.email,user_password:this.state.password})
             if (res.status == 200) {
                 this.setState({
                     loading: false,
                     snackbar: true,
                        ...
                        <div className="container">
                            <div className="header-body text-center mb-7">
                                <div className="row justify-content-center">
                                    ...
                                    <h2 style={{ paddingTop: '40px' }}>Shubham Mishra(1BY18CS162) &</h2>
                                    <h2>Subbhashit Mukherjee(1BY18CS171)</h2>
