...
class SelectComp extends React.Component {
    ...
    componentDidMount() {
        axios.get('http://localhost:8000/getallcomplaints')
            .then(response => {
                this.setState({
    ...
    async setComp(compId) {
        try {
            console.log(localStorage.getItem('admin_name')+"      "+localStorage.getItem('admin_id')+"   "+compId)
            var res = await axios.post('http://localhost:8000/selectcomplaint', { admin_name: localStorage.getItem('admin_name'), admin_id: localStorage.getItem('admin_id'), complaint_id:compId})
            if (res.status == 200) {
                this.setState({
                    loading: false,
                    snackbar: true,
                        ...
                        <div class="container-fluid d-flex align-items-center">
                            <div class="row">
                                <div class="col-lg-7 col-md-10" style={{ paddingBottom: '20px' }}>
                                    <div style={{ display: 'inline-block' }}><i class="fas fa-layer-group" style={{ fontSize: '2.5rem', color: 'white', paddingRight: '10px' }}></i></div><div style={{ display: 'inline-block' }}><h1 class="display-2 text-white">Select Complaints</h1></div>
                                </div>
                            </div>
                        </div>
                    </div>
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div className="col-4a text-right">
                                                            <button onClick={(e) => { this.submit(e,comp.COMPLAINT_ID)}} className="btn btn-primary my-3" style={{ fontSize: "12px" }}>Choose Complaint</button>
                                                        </div>
                                                    </td>
                                                </tr>
                                            </tbody>
